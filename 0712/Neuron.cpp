...
class Neuron
{
    public:
        ...
        double getLogisticCost(double input, double output)
        {
            double sigma = feedForward(input);
        ...
        double getTotalLogisticCost(vector<double> X, vector<double> Y)
        {
            int n = static_cast<int>(X.size());
            double cost = 0;

            for(int i = 0; i<n; i++){
                cost += getLogisticCost(X[i],Y[i]);
            }
...
int main()
{
    ...
    for(int i = 0; i<100000; i++){
        if(i%10000 == 0){
            printf("Training : %d  \n",i);
            N.feedForwardAndPrint(50);
            printf("w1 = %f, w2 = %f, wb = %f  \n\n", N.w1, N.w2, N.wb);
            //printf("Cost = %f \n\n", cost);
        }
        //cost = N.getTotalLogisticCost(N3.X, N3.Y);
        N.backProp(N.X1, N.X1, N.Y);
    }
